...
#修改密码
@pytest.mark.login
def test_denglu_changepwd(pub_data):
    pub_data["newPwd"] = "aaa123"
    pub_data["reNewPwd"] = "aaa123"
    pub_data["oldPwd"] = "asdad212212"
  ...
  "userName": "${username}"
    ...
    # --------------------分界线，下边的不要修改-----------------------------------------
    # method,pub_data和url为必传字段
    r = request_tool.request(method=method,url=uri,pub_data=pub_data,status_code=status_code,headers=h,expect=expect,feature=feature,story=story,title=title,json_data=json_data)
...
# 登录验证
@pytest.mark.login
def test_denglu_login(pub_data):
    pub_data["pwd"]= "aaa123"
    method = "POST"  #请求方法，全部大写
    feature = "用户模块"  # allure报告中一级分类
...
def test_withdraw(pub_data):
    pub_data["cardNo"] = "自动生成 字符串 16 数字 "
    method = "POST"  #请求方法，全部大写
    feature = "用户模块"  # allure报告中一级分类
